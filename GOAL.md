...
| 2026-05-28 18:43 EDT | CP7 — OAuth authorize repair | Reconciled the Anthropic browser URL with Claude Code's bundled OAuth builder: added `code=true`, switched the Anthropic redirect to `http://localhost:1455/callback`, included `org:create_api_key` in the Claude Code scope set, and kept `/auth/callback` as a listener alias for OpenAI/back-compat. | User-visible Claude authorization failure → fresh Anthropic OAuth starts with Claude-Code-shaped authorize params; OpenAI OAuth URL remains unchanged and `app/modules/proxy/service.py` remains untouched. | Restart the local CP7 server, open a fresh corrected Claude login tab, then continue real-account verification after user consent. |
