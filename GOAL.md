...
**Current checkpoint:** CP7 — real-account login/runtime verification
...
**Current value:** CP7 runtime hardening is in progress: the dev server is running on `http://127.0.0.1:2456`, the fresh Anthropic browser OAuth flow is open in Chrome, and the callback listener is active on `127.0.0.1:1455`. Real-account smoke found and fixed three live-runtime issues: inbound `x-api-key` is no longer forwarded upstream, the Anthropic stream route now returns a proper 503 JSON error before streaming when no account is available, and the Anthropic OAuth authorize URL now matches Claude Code's browser flow (`code=true`, `/callback`, and the Claude Code scope set). One imported Anthropic account is currently rate-limited and another is paused with stale credentials, so Stage B still requires the user to complete fresh Claude login for at least two accounts.
**Last verified:** 2026-05-28 18:43 EDT — `uv run pytest -q tests/unit/test_provider_registry.py tests/unit/test_anthropic_oauth.py tests/integration/test_oauth_flow.py` passed (30 passed); `uv run pytest -q tests/integration/test_anthropic_proxy.py tests/unit/test_auth_manager.py tests/unit/test_provider_registry.py tests/unit/test_anthropic_oauth.py tests/unit/test_model_refresh_scheduler.py tests/unit/test_usage_updater.py tests/unit/test_usage_refresh_scheduler_recovery.py tests/unit/test_proxy_load_balancer_refresh.py tests/integration/test_oauth_flow.py` passed on rerun (162 passed, 3 skipped; first broad run hit an OpenAI device-OAuth timing flake that passed in isolation and in the full OAuth file); `uv run ruff check app/modules/oauth/service.py app/core/providers/anthropic.py app/core/config/settings.py tests/unit/test_provider_registry.py tests/integration/test_oauth_flow.py` passed; `git diff -- app/modules/proxy/service.py` and `git diff --check` empty. Live Claude CLI smoke correctly reaches `/v1/messages` through `ANTHROPIC_BASE_URL`, but returns 503 until fresh real accounts are available.
**Remaining:** CP7–CP8
**Blocked:** Waiting for user login/consent in the open Anthropic OAuth tab, then Stage B distribution/failover/cost verification can continue.
...
| 2026-05-28 18:03 EDT | CP6 — Frontend dashboard | Added provider-aware account schemas, provider badges in list/detail, provider search, OpenAI/Anthropic OAuth selector, Claude browser-only OAuth guard, and Anthropic cache creation/read usage display. | Unified dashboard OpenAI-only → dashboard can list both providers, start Anthropic OAuth, and render Anthropic cache-token cost signals; Claude accounts still 0 until user completes CP7 login. | Open local Anthropic login flow for user; then run Claude Code runtime checks with real accounts. |
| 2026-05-28 18:32 EDT | CP7 — Runtime login setup | Ran the dual-provider app on `127.0.0.1:2456`, opened a fresh Anthropic Claude OAuth browser flow with Claude-Code scopes, confirmed callback listener on `127.0.0.1:1455`, and hardened live-runtime behavior for Anthropic token refresh, OpenAI-only background jobs, upstream API-key filtering, and no-account streaming errors. | Runtime smoke unknown → Claude Code reaches `/v1/messages` through `ANTHROPIC_BASE_URL`; invalid dummy `x-api-key` is no longer leaked upstream; unavailable-account failures return honest 503 JSON. Stage B remains incomplete because current imported Claude accounts are rate-limited/paused. | User completes the open Claude login flow for at least two accounts; then rerun ≥20 Claude Code requests and verify distribution, 429 failover, cost, and OpenAI regression. |
| 2026-05-28 18:48 EDT | CP7 — OAuth authorize repair | Reconciled the Anthropic browser URL with Claude Code's bundled OAuth builder: added `code=true`, included `org:create_api_key` in the Claude Code scope set, and accepted Claude Code manual `code#state` callbacks. A live Claude page still rejected the localhost redirect, so Anthropic now uses Claude Code's manual `https://platform.claude.com/oauth/code/callback` redirect while OpenAI keeps the local callback. | User-visible Claude authorization failure → Anthropic OAuth now uses the same fallback callback URL printed by `claude auth login --claudeai`; OpenAI OAuth URL remains unchanged and `app/modules/proxy/service.py` remains untouched. | Restart the local CP7 server, open a fresh corrected Claude login tab, have the user paste the displayed code into the dashboard, then continue real-account verification. |
| 2026-05-28 18:54 EDT | CP7 — Claude Code URL parity | Matched the remaining Claude Code authorize shape exactly: `code=true` appears first, Anthropic state tokens use 32 bytes/43 URL-safe chars, and the CP7 server was restarted with the updated URL builder. | Post-authorize `Invalid request format` → fresh CDP run reached Claude with full state preserved and CLI-shaped query; focused backend tests and ruff pass; Chrome is now stopped at Cloudflare human verification, which requires user action. | User completes Cloudflare/Claude authorization in Chrome, then paste the returned `code#state` into the dashboard for token exchange and runtime verification. |
