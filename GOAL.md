...
**Current value:** CP7 runtime hardening is in progress: the dev server is running on `http://127.0.0.1:2456`, the fresh Anthropic browser OAuth flow is open in Chrome, and the callback listener is active on `127.0.0.1:1455`. Real-account smoke found and fixed two live-runtime issues: inbound `x-api-key` is no longer forwarded upstream, and the Anthropic stream route now returns a proper 503 JSON error before streaming when no account is available. One imported Anthropic account is currently rate-limited and another is paused with stale credentials, so Stage B still requires the user to complete fresh Claude login for at least two accounts.
**Last verified:** 2026-05-28 18:32 EDT — `uv run pytest -q tests/integration/test_anthropic_proxy.py tests/unit/test_auth_manager.py tests/unit/test_provider_registry.py tests/unit/test_anthropic_oauth.py tests/unit/test_model_refresh_scheduler.py tests/unit/test_usage_updater.py tests/unit/test_usage_refresh_scheduler_recovery.py tests/unit/test_proxy_load_balancer_refresh.py` passed (138 passed, 3 skipped); `uv run ruff check app/modules/proxy/anthropic_service.py tests/integration/test_anthropic_proxy.py app/modules/accounts/auth_manager.py app/core/providers/types.py app/core/providers/openai.py app/core/providers/anthropic.py app/core/anthropic/oauth.py app/core/config/settings.py app/core/openai/model_refresh_scheduler.py app/core/usage/refresh_scheduler.py app/modules/usage/updater.py app/modules/proxy/load_balancer.py tests/unit/test_auth_manager.py tests/unit/test_provider_registry.py tests/unit/test_anthropic_oauth.py tests/unit/test_model_refresh_scheduler.py tests/unit/test_usage_updater.py tests/unit/test_usage_refresh_scheduler_recovery.py tests/unit/test_proxy_load_balancer_refresh.py` passed; `git diff -- app/modules/proxy/service.py` and `git diff --check` empty. Live Claude CLI smoke correctly reaches `/v1/messages` through `ANTHROPIC_BASE_URL`, but returns 503 until fresh real accounts are available.
**Remaining:** CP7–CP8
