...
def _usage_history_from_sqlite_row(row) -> UsageHistory:
    return UsageHistory(
        ...
        credits_cap=float(row[12]) if len(row) > 12 and row[12] is not None else None,
        credits_spent=float(row[13]) if len(row) > 13 and row[13] is not None else None,
    )
