...
```bash
helm dependency build deploy/helm/agent-lb/
helm upgrade --install agent-lb deploy/helm/agent-lb/ \
  -f deploy/helm/agent-lb/values-bundled.yaml \
  --set postgresql.auth.password=change-me
```
...
```bash
helm upgrade --install agent-lb deploy/helm/agent-lb/ \
  -f deploy/helm/agent-lb/values-external-db.yaml \
  --set externalDatabase.url='postgresql+asyncpg://user:pass@db.example.com:5432/agentlb'
```
...
```bash
helm upgrade --install agent-lb deploy/helm/agent-lb/ \
  -f deploy/helm/agent-lb/values-external-secrets.yaml \
  --set externalSecrets.secretStoreRef.name=my-store
```
...
When `externalDatabase.existingSecret` is set and `auth.existingSecret` is not, the chart-managed app secret contains only the encryption key; the StatefulSet reads `AGENT_LB_DATABASE_URL` from the external DB secret.
...
- StatefulSet name: `<release>-agent-lb-workload`
- `serviceName: <release>-agent-lb-bridge` on the StatefulSet
- `AGENT_LB_HTTP_RESPONSES_SESSION_BRIDGE_INSTANCE_ID=$(POD_NAME)`
- `AGENT_LB_HTTP_RESPONSES_SESSION_BRIDGE_ADVERTISE_BASE_URL=http://$(POD_NAME).<headless-service>.$(POD_NAMESPACE).svc.<clusterDomain>:2455`
...
```yaml
config:
  sessionBridgeAdvertiseBaseUrl: "http://agent-lb-internal.default.svc.cluster.local:2455"
```

When `networkPolicy.enabled=true`, the chart also allows port `2455` traffic between agent-lb pods so owner handoff can work without extra rules.
...
```yaml
config:
  sessionBridgeInstanceRing: "agent-lb-0.agent-lb.default.svc.cluster.local,agent-lb-1.agent-lb.default.svc.cluster.local"
```
...
```bash
helm test agent-lb -n <namespace>
...
```bash
kubectl port-forward svc/agent-lb 2455:2455 -n <namespace>
...
```bash
kubectl describe pod -l app.kubernetes.io/name=agent-lb -n <namespace>
