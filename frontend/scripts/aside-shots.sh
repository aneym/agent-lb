...
# The app's email blur is switched on for the origin and email-shaped text is masked before capture,
# so the images can be shared on the private review site.
set -eu
...
for path in "$@"; do
  ...
  js=$(cat <<JS
await attachBrowserTab('$id');
await fs.mkdir('./artifacts',{recursive:true});
const res={};
await page.evaluate(()=>localStorage.setItem('agent-lb-privacy','1'));
const mask=()=>page.evaluate(()=>{ const d=document.getElementById('shot').contentDocument; const re=/[A-Za-z0-9._%+-]+@[A-Za-z0-9-]+(\.[A-Za-z0-9-]+)+/g;
  const tw=d.createTreeWalker(d.body, NodeFilter.SHOW_TEXT); let n; while((n=tw.nextNode())) if(re.test(n.nodeValue)) { re.lastIndex=0; n.nodeValue=n.nodeValue.replace(re, m=>m[0]+'•••@•••'); }
  d.querySelectorAll('[title]').forEach(e=>{ if(re.test(e.title)) { re.lastIndex=0; e.title=''; } }); });
for (const w of [1440,390]) for (const theme of ['light','dark']) {
  const vw = await page.evaluate(()=>innerWidth);
  const s = Math.min(1, (vw-4)/w);
  await page.evaluate(({p,w,s})=>{ document.head.innerHTML=''; document.body.innerHTML=''; document.documentElement.className='';
    document.documentElement.style.cssText='margin:0;overflow-x:hidden'; document.body.style.cssText='margin:0';
    const f=document.createElement('iframe'); f.id='shot'; f.setAttribute('scrolling','no'); f.src=p; f.style.cssText='border:0;display:block;width:'+w+'px;height:900px;transform-origin:0 0;transform:scale('+s+')';
    document.body.appendChild(f); }, {p:'$path', w, s});
  await sleep(7000);
  const info = await page.evaluate(({theme,s})=>{ const f=document.getElementById('shot'); const d=f.contentDocument;
    d.documentElement.classList.toggle('dark', theme==='dark');
    const h=Math.max(d.documentElement.scrollHeight, d.body.scrollHeight); f.style.height=h+'px'; document.body.style.height=Math.ceil(h*s)+'px';
    return {h, s, sw:d.documentElement.scrollWidth, cw:d.documentElement.clientWidth, url:f.contentWindow.location.pathname, vh:innerHeight, dpr:devicePixelRatio}; }, {theme,s});
  await sleep(1200);
...
await fs.writeFile('./artifacts/$slug-checks.json', JSON.stringify(res));
await page.close();
JS
)
