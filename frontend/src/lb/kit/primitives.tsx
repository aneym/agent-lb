...
import { clock, dayTime, duration, pace, pct } from "../format";
...
export function LimitMeter({
  ...
  const percent = Math.min(100, Math.max(0, remaining ?? 0));
  return (
    <div className={`lim ${valid ? "" : "na"}`}>
      <div className="lt">
        <span className={`v ${valid && percent < 20 ? "warn" : ""}`}>
          {valid ? (
            <>
              {percent < 20 && "△ "}
              {pct(percent)}
          ...
          {valid && label && <span className="u">{label}</span>}
        </span>
        {resetAt && (
          <span className="r">
            {label === "5h" ? duration(new Date(resetAt).getTime() - now) : dayTime(resetAt)}
          </span>
        )}
      </div>
      <div className="bar">
        {valid && <i style={{ width: `${percent}%` }} />}
        {paceTick != null && (
          <b
            style={{ left: `${Math.min(100, Math.max(0, paceTick))}%` }}
            title="Expected remaining at this point"
          />
        )}
      </div>
    </div>
...
export function AccountState({
  ...
  const suffix = resetAt ? clock(resetAt) : "reset";
  return (
    <div className="qs">
      <span
...
export function Pace({ value }: { value?: number | null }) {
  return (
    <div className="pace">
      <span className="num">{pace(value)}</span>
    </div>
  );
}
