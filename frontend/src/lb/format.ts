...
export const clock = (value: string | number | Date) => new Intl.DateTimeFormat("en-GB", { hour: "2-digit", minute: "2-digit", hour12: false }).format(date(value));
export const dayTime = (value: string | number | Date) => new Intl.DateTimeFormat("en-GB", { weekday: "short", hour: "2-digit", minute: "2-digit", hour12: false }).format(date(value));
export const resetText = (value: string | number | Date) => `resets in ${duration(date(value).getTime() - Date.now())} · ${clock(value)}`;
...
export const money = (value: number) => new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", maximumFractionDigits: Math.abs(value) >= 1 ? 0 : 2 }).format(value);
export function pace(value: number | null | undefined) {
  if (value == null) return "—";
  if (Math.abs(value) < .5) return "on pace";
