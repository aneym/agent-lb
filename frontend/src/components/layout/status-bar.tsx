...
function getRoutingLabel(
  ...
  if (strategy === "capacity_weighted") {
    if (sticky && preferEarlier)
      return `Capacity weighted + Sticky + ${earlyResetLabel}`;
  ...
  if (strategy === "relative_availability") {
    return sticky
      ? "Relative availability + Sticky threads"
      : "Relative availability";
  }
  if (strategy === "fill_first") {
    if (sticky && preferEarlier)
      return `Fill first + Sticky + ${earlyResetLabel}`;
...
export function StatusBar() {
  const { data: lastSyncAt = null } = useQuery({
    ...
    queryFn: () =>
      getDashboardOverview({ timeframe: DEFAULT_OVERVIEW_TIMEFRAME }),
  ...
  const showUpdateAvailable =
    runtimeVersion?.updateAvailable === true && latestVersion;
  ...
  return (
    <footer className="fixed bottom-0 left-0 right-0 z-50 border-t border-border bg-background px-4 py-2">
      <div className="mx-auto flex w-full max-w-[1500px] items-center gap-4 text-xs text-muted-foreground">
        <div className="flex min-w-0 flex-wrap items-center gap-x-5 gap-y-1">
          <span className="inline-flex items-center gap-1.5">
            {isLive ? (
              <span
                className="h-1.5 w-1.5 animate-pulse rounded-full bg-foreground motion-reduce:animate-none"
                title="Live"
              />
            ) : (
            ...
            <span className="font-mono tabular-nums">{lastSync.time}</span>
          </span>
          <span className="inline-flex items-center gap-1.5">
          ...
          <span className="inline-flex items-center gap-1.5">
            ...
            {showUpdateAvailable ? (
              <a
                ...
                className="inline-flex h-4 w-4 items-center justify-center rounded-full text-foreground transition-colors duration-150 ease-out hover:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
        ...
        <a
          ...
          className="ml-auto inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-border bg-background text-muted-foreground transition-colors duration-150 ease-out hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
