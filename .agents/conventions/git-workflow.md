...
> **Fork policy override (`aneym/agent-lb`)**: development on this fork happens **directly on `main`** — no feature branches unless explicitly requested. Rules 2 and 7 below (branch naming, "do not commit directly to main") apply only when contributing to upstream `Soju06/codex-lb`. See "Branch Policy (fork)" in `AGENTS.md`.
