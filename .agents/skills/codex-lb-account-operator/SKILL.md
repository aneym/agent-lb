---
name: "codex-lb-account-operator"
description: |
  Manage codex-lb account subscription/admin work with local browser profiles,
...
# Codex LB Account Operator

Use this skill when the user asks to add, cancel, pause, reactivate, verify, or
open an OpenAI/ChatGPT/Anthropic account that codex-lb routes through.
...
```
.codex-lb/account-profiles.json
```
...
```
.agents/skills/codex-lb-account-operator/account-profiles.example.json
```

Use the registry only as local operator memory. The codex-lb app database remains
the dashboard source of truth for account status, routing pause/reactivate state,
and subscription ledger fields.

## Operating Model

1. Read `.codex-lb/account-profiles.json` before account-management work.
...
6. Update codex-lb's subscription ledger after each human/browser billing action.
...
```
.codex-lb/chrome-profiles/<profileHandle>
```

Open a profile on macOS with:

```bash
open -na "Google Chrome" --args --user-data-dir="$PWD/.codex-lb/chrome-profiles/<profileHandle>" "https://chatgpt.com"
```
...
Use these statuses in the codex-lb dashboard/API ledger:
...
- Distinguish vendor subscription state from codex-lb routing state. Pausing an
  account in codex-lb only removes it from routing; it does not cancel vendor
  billing.
