...
#### Scenario: Failed seat-guard process

- **GIVEN** the installed `seat-guard` script exits unsuccessfully
- **WHEN** Claude Code executes the registered PreToolUse command
- **THEN** the command emits a `deny` hook decision on standard output
