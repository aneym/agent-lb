...
The system SHALL provide a `ccdex` command that executes the installed Claude Code harness while routing inference through agent-lb's dedicated Codex compatibility path, launching with permission mode `bypassPermissions` unless the operator explicitly supplies a permission option.
...
- **WHEN** an operator runs `ccdex` with `--permission-mode <mode>` or `--dangerously-skip-permissions`
