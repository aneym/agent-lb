...
## Why
agent-lb currently pools ChatGPT/OpenAI accounts only. Operators who also use Claude Code cannot point `ANTHROPIC_BASE_URL` at the same local balancer because there is no Anthropic provider, no Anthropic OAuth flow, and no Messages API proxy path.

The goal is to make one local agent-lb instance manage both account pools while preserving the existing OpenAI/Codex request path for upstream reviewability.
...
## Impact
- Claude Code can use agent-lb through `ANTHROPIC_BASE_URL` once real Anthropic OAuth accounts are added.
