...
Claude Code load-balanced launch must preserve the client billing mode. Setting only `ANTHROPIC_BASE_URL` points Claude Code at codex-lb while keeping Claude Max/OAuth billing visible in the client. Setting `ANTHROPIC_AUTH_TOKEN` makes Claude Code switch to API Usage Billing and is not acceptable for the local launcher or default rollout.
