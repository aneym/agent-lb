...
Operators and client integrations need a stable way to identify which agent-lb build produced a successful or client-error HTTP response. Today the service exposes request identifiers and some route-specific headers, but it does not expose the running application version on normal HTTP responses.
...
- Define the header value as the running agent-lb package version rather than the stale FastAPI metadata version.
...
- `api-response-metadata`: global HTTP response headers that agent-lb adds across route families.
...
- **Behavior**: HTTP `200-499` responses now include `X-App-Version` with the running agent-lb package version
