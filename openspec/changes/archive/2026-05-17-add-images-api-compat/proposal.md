...
OpenAI clients (`openai` SDK, downstream tools, Codex CLI fallback) call `POST /v1/images/generations` and `POST /v1/images/edits` to access GPT Image models such as `gpt-image-2`. Today agent-lb does not expose those endpoints, so any client that wants to use ChatGPT-backed image generation through agent-lb has no entrypoint. Direct calls to `https://api.openai.com/v1/images/...` require a platform organization on the ChatGPT account (token-exchange to `openai-api-key` returns `invalid ID token: missing organization_id` for typical Plus/Pro accounts), which is a separate platform onboarding most ChatGPT users do not have.
...
- Account `image_generation` token usage from `response.tool_usage.image_gen` against agent-lb's existing usage history with a new request kind for image generation.
...
- New OpenAI-compatible image surface on agent-lb without requiring users to onboard to platform.openai.com.
...
- `codex/scripts/image_gen.py` CLI fallback can target agent-lb by setting `OPENAI_BASE_URL` to the agent-lb base and `OPENAI_API_KEY` to a agent-lb-issued key. No upstream codex changes are required.
