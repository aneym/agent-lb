...
- Clients (Codex CLI, OpenCode, OpenClaw) now see a retry hint bounded by 300s when agent-lb has zero selectable accounts. Their next retry will land inside agent-lb's auto-recovery window (60s refresh + 120s cooldown buffer + margin), so transient `/wham/usage` divergence after OpenAI reset events resolves on the next attempt instead of stalling the client for hours.
