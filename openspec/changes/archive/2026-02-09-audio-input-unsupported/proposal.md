...
ChatGPT backend rejects audio file inputs (unsupported file type), and agent-lb currently allows chat `input_audio` which fails downstream with inconsistent errors. We also need to align file_id handling with upstream by allowing pass-through instead of local rejection.
