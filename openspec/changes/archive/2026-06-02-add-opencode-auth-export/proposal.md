...
## Why
Operators who use agent-lb as their account onboarding dashboard currently need to authenticate a ChatGPT/OpenAI account in agent-lb and then repeat the OpenAI OAuth flow in OpenCode. This duplicates setup work even though agent-lb already stores the account tokens encrypted after import or OAuth.
...
## Impact
- Users can authenticate once in agent-lb, export a selected account, and copy the resulting payload into OpenCode's `~/.local/share/opencode/auth.json`.
