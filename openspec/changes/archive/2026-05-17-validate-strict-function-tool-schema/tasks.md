...
- [x] T12: Targeted sweep (per `agent-lb-development` skill §5): 241 passed.
...
- [x] T15: Live four-cell matrix against the local docker container (`agent-lb-dev-server`) with a real account: 8/8 PASS (A→400 with correct code+param on both endpoints; B/C/D→200 on both).
