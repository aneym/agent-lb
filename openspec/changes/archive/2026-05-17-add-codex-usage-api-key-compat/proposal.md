...
## Why
OpenCodeBar calls `/api/codex/usage` with the same LB API key it uses for `/v1/*` proxy calls. In 1.13.x that route only accepts ChatGPT caller identity (`chatgpt-account-id` + ChatGPT token), so API-key clients fail with `Missing chatgpt-account-id header` even though agent-lb already has self-service usage data for API keys.
