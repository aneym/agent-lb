...
`agent-lb`'s public `/v1/responses` endpoint is advertised as OpenAI-compatible (README: "OpenAI Python SDK", "OpenCode", "OpenClaw" all point at `/v1`). But the streaming path forwards the upstream Codex backend's SSE event stream with only per-event normalization — it never reconciles the *stream-level shape* the OpenAI SDK parser requires. An audit against the OpenAI Python SDK (`openai>=2.16`) `ResponseStreamState` parser found two stream-level contract gaps:
