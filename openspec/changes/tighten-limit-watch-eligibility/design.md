...
- Keep missing `lastRefreshAt` observable but non-fatal as requested. Invalid or stale supplied values fail closed.

## Risks / Trade-offs

- [Additional quota shape changes] → Missing or malformed Fable data excludes only Fable admission and is covered by fixtures.
- [More accounts excluded after deployment] → The snapshot reasons identify the failure class with truncated account IDs.
