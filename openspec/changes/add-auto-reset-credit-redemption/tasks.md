...
- [x] 5. `ruff` + tests green; deployed to both instances; guard behavior verified live (active accounts present → no redemption)
- [x] 6. Expiry sweep: settings, hourly sweep redeeming credits within the expiry window (`nothing_to_reset` retries safely), `trigger: expiring` audit, post-redemption wake probe, unit tests, live verification against tonight's expiring credits (both redeemed; accounts recovered to active/100%)
