...
can route per-request by model string. The CCGPT bridge (locked Sol profile)
...
- `/v1/messages` recognizes Sol worker aliases (`gpt-5.6-sol`, plus
  ...
  the existing CCGPT bridge instead of the Anthropic pool. Alias-pinned effort
...
- `/v1/ccgpt/messages` behavior is unchanged; it now shares one helper with
  the alias path.
...
- Affected code: `app/modules/proxy/api.py`, `tests/integration/test_ccgpt_proxy.py`.
