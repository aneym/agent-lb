...
- [ ] Settings: `anthropic_fable_over_threshold_probe_enabled` (default true),
      `anthropic_fable_probe_ttl_seconds` (default 43200)
      (`app/core/config/settings.py`).
- [ ] Pulse: Fable probe for routable Anthropic accounts at/over the weekly
...
- [ ] Eligibility: over-threshold accounts with a fresh capable marker join the
      Fable pool in `_provider_quota_eligibility`
      (`app/modules/proxy/anthropic_service.py`).
- [ ] Tests: capable marker admits an over-threshold account to Fable routing;
...
- [ ] Validate: pytest, ruff, `openspec validate`, service restart + live check.
