...
The Agent PreToolUse guard SHALL treat every non-forwarder seat as Anthropic volume, including unknown or empty seat types. A named non-Anthropic forwarder seat SHALL remain non-Anthropic only when it has no explicit Anthropic model parameter; explicit Opus, Sonnet, Fable, Haiku, or Claude model parameters SHALL be Anthropic volume. An explicit model parameter containing `fable` on a subagent SHALL always deny, regardless of snapshot health or SEAT_GUARD_ALLOW_ANTHROPIC. The guard SHALL normalize subagent type and model parameters by stripping whitespace and lowercasing before comparison. It SHALL deny Anthropic volume when the configured limit-watch snapshot is missing, unparseable, older than 600 seconds, more than 60 seconds in the future, unreachable, or reports fewer than two Anthropic usable or Fable-eligible usable accounts. Guard errors, including top-level initialization failures, unparseable stdin, non-object payloads, and malformed Agent input, SHALL deny the dispatch. A non-Agent string tool name SHALL remain harmless and return silently. Route-table failures SHALL not bypass the Anthropic snapshot check or deny a non-Anthropic forwarder. An explicit SEAT_GUARD_ALLOW_ANTHROPIC=1 override MAY admit Anthropic volume other than an explicit Fable pin only when its ledger record is successfully appended. `fork` with no model SHALL remain allowed and recorded with `"fork": true`; a fork with an explicit Anthropic model SHALL undergo the snapshot check.
