...
Update after the deployment closeout: the isolated worktree's verification prerequisites were repaired, and the exact full `uv run pytest -q` now passes with 4,344 passed and 43 skipped. Ruff lint, full application formatting, frontend checks, build, and isolated SQLite migration/schema checks pass. See `../repair-team-mode-verification/context.md` for current results. The older failures below are retained as historical deployment evidence. The follow-up repairs remain isolated and uncommitted, not deployed. The member-PC SSH acceptance is still pending.

## Result

Team mode is enabled on `https://host.example.ts.net:2455/team`. The member has one active key and aggregate caps of $5/day, $20/week, and $50/month. Windows and zsh onboarding snippets use that HTTPS origin. The key is not present in this repository or the evidence files.
...
Live OpenAI `gpt-5.6-luna` and Anthropic `claude-haiku-4-5` requests both returned `TEAM_MODE_OK` using the real member key. The ledger attributed 18 OpenAI tokens and 24 Anthropic tokens to the member. Sonnet returned an upstream account-pool quota error, not a team gate denial.
...
Aside exercised the isolated backend through the real dashboard: create member, invalid cap validation, saved caps, key issuance, key count, and onboarding dialog. Aside also opened the deployed HTTPS Team page and confirmed the member's row. Screenshots are in evidence/.
...
- Untrusted forwarded member IP without a key: HTTP 401.
- Untrusted forwarded member IP accessing dashboard routes: HTTP 403.
- Same IP with the member's key: models and self-service usage HTTP 200.
- Suspended member: HTTP 403 `team_member_suspended`.
...
- Both provider inference checks returned the expected text and were attributed to the member.

These member-IP checks use the real local ingress with an explicit `X-Forwarded-For` header, not a request originating on the member's machine. Trusted HTTPS dashboard access was exercised over the actual Tailscale URL.

## Deployment and recovery

Runtime backup and reviewed hash manifest: `~/.agent-lb/runtime/backups/team-mode-20260919T121049/`.
...
The member is online at `member.example.ts.net`, but SSH port 22 times out. The username and an enabled SSH server are pending user input. There is no claim that the member's actual PC has run either CLI.

Prepared outside the repository:

- `~/.agent-lb/team/member.key`, permissions 0600.
- `~/.agent-lb/team/member-setup.ps1`, prompts for the key and sets user-scoped Claude Code/Codex environment variables.
- `~/.agent-lb/team/member-onboarding.txt`, key-free instructions.

The server feature is operational. Member-machine acceptance and the unrelated full-repository failures are not resolved by this closeout.
