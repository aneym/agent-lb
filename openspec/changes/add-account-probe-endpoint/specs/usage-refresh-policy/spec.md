...
- **THEN** the service sends one `responses.create` request directly to `{upstream_base_url}/codex/responses` with `stream=true`, `store=false`, and no `max_output_tokens` parameter (the upstream rejects it with 400 "Unsupported parameter" as of 2026-07-01)
