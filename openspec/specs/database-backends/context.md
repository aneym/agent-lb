...
agent-lb is designed to be SQLite-first for simple local usage and container defaults. SQLite-specific resilience behavior (integrity checks, WAL tuning, recovery tooling) remains valuable for the default mode.
...
- Accept PostgreSQL through `AGENT_LB_DATABASE_URL` only; no new configuration key aliases.
...
- SQLite default URL: `sqlite+aiosqlite:///~/.agent-lb/store.db`
- SQLite startup check mode: `AGENT_LB_DATABASE_SQLITE_STARTUP_CHECK_MODE=quick|full|off` (default `quick`)
- PostgreSQL example URL: `postgresql+asyncpg://agent_lb:agent_lb@127.0.0.1:5432/agent_lb`
...
```bash
AGENT_LB_DATABASE_URL=postgresql+asyncpg://agent_lb:agent_lb@127.0.0.1:5432/agent_lb agent-lb
```

Use SQLite with explicit full startup validation:

```bash
AGENT_LB_DATABASE_SQLITE_STARTUP_CHECK_MODE=full agent-lb
```
