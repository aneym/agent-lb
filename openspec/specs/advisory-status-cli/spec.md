# advisory-status-cli Specification

## Purpose
Define the read-only local status command's account and model observations so operators can distinguish known capacity from missing telemetry without changing upstream or routing state.
