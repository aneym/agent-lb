...
### Requirement: Ccgpt provider ownership
Agent-lb MUST route every authenticated `ccgpt` compatibility request only through eligible OpenAI accounts and MUST preserve existing OpenAI session/cache affinity, failover, settlement, and API-key policy behavior.

#### Scenario: Mixed provider pool
- **WHEN** both OpenAI and Anthropic accounts are eligible and a `ccgpt` request arrives
...
### Requirement: Ccgpt credential isolation
