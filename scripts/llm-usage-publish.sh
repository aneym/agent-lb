...
# ~/Library/LaunchAgents/com.example.llm-usage-publish.plist (StartInterval 900).
set -euo pipefail
...
# Git URL of the usage repo; needed only for the first clone. No default.
REMOTE="${LLM_USAGE_REMOTE:-}"
...
if [ ! -d "$REPO_DIR/.git" ]; then
  if [ -z "$REMOTE" ]; then echo "$(date -u +%FT%TZ) error: set LLM_USAGE_REMOTE to clone the usage repo" >>"$LOG"; exit 1; fi
  git clone "$REMOTE" "$REPO_DIR" >>"$LOG" 2>&1
fi
cd "$REPO_DIR"
