...
async def test_backpressure_returns_429_when_at_capacity():
    ...
    assert overloaded.json() == {"detail": "agent-lb is temporarily overloaded by local backpressure"}
...
async def test_backpressure_dashboard_websocket_uses_detail_payload_when_at_capacity():
    ...
    assert app_called is False
    payload = json.loads(cast(bytes, sent_events[1]["body"]).decode("utf-8"))
    assert payload == {"detail": "agent-lb is temporarily overloaded by local backpressure"}
