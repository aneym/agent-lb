...
async def test_trigger_restart_uses_configured_kubectl_binary(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    config = verify_script.VerifyConfig(
        ...
        rollout_namespace="agent-lb-e2e",
        rollout_context="kind-agent-lb-local",
        rollout_target="statefulset/agent-lb-e2e-workload",
        delete_owner_after_first_turn=False,
        postgresql_pod="postgresql-0",
        postgresql_user="agentlb",
        postgresql_database="agentlb",
    ...
    assert seen["cmd"] == [
        ...
        "rollout",
        "restart",
        "statefulset/agent-lb-e2e-workload",
    ]
