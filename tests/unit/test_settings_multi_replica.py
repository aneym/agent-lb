...
def test_settings_metrics_port_from_env(monkeypatch):
    monkeypatch.setenv("AGENT_LB_METRICS_PORT", "8080")
    settings = Settings()
    assert settings.metrics_port == 8080


def test_settings_rejects_metrics_port_2455(monkeypatch):
    monkeypatch.setenv("AGENT_LB_METRICS_PORT", "2455")
...
def test_settings_conversation_archive_queue_max_bytes_from_env(monkeypatch):
    monkeypatch.setenv("AGENT_LB_CONVERSATION_ARCHIVE_QUEUE_MAX_BYTES", "16777216")
    settings = Settings()
...
def test_settings_circuit_breaker_recovery_timeout_from_env(monkeypatch):
    monkeypatch.setenv("AGENT_LB_CIRCUIT_BREAKER_RECOVERY_TIMEOUT_SECONDS", "120")
    settings = Settings()
...
def test_settings_backpressure_max_concurrent_requests_from_env(monkeypatch):
    monkeypatch.setenv("AGENT_LB_BACKPRESSURE_MAX_CONCURRENT_REQUESTS", "50")
    settings = Settings()
    assert settings.backpressure_max_concurrent_requests == 50


def test_settings_split_bulkhead_limits_from_env(monkeypatch):
    monkeypatch.setenv("AGENT_LB_BULKHEAD_PROXY_HTTP_LIMIT", "40")
    monkeypatch.setenv("AGENT_LB_BULKHEAD_PROXY_WEBSOCKET_LIMIT", "25")
    monkeypatch.setenv("AGENT_LB_BULKHEAD_PROXY_COMPACT_LIMIT", "8")
    settings = Settings()
...
def test_settings_split_bulkhead_limits_allow_explicit_zero(monkeypatch):
    monkeypatch.setenv("AGENT_LB_BULKHEAD_PROXY_HTTP_LIMIT", "0")
    monkeypatch.setenv("AGENT_LB_BULKHEAD_PROXY_WEBSOCKET_LIMIT", "0")
    monkeypatch.setenv("AGENT_LB_BULKHEAD_PROXY_COMPACT_LIMIT", "0")
    settings = Settings()
...
def test_settings_work_admission_limits_from_env(monkeypatch):
    monkeypatch.setenv("AGENT_LB_PROXY_TOKEN_REFRESH_LIMIT", "7")
    monkeypatch.setenv("AGENT_LB_PROXY_UPSTREAM_WEBSOCKET_CONNECT_LIMIT", "9")
    monkeypatch.setenv("AGENT_LB_PROXY_RESPONSE_CREATE_LIMIT", "11")
    monkeypatch.setenv("AGENT_LB_PROXY_COMPACT_RESPONSE_CREATE_LIMIT", "3")
    settings = Settings()
...
def test_settings_upstream_websocket_proxy_env_defaults_to_direct_when_unset(monkeypatch):
    ...
    monkeypatch.delenv("AGENT_LB_UPSTREAM_WEBSOCKET_TRUST_ENV", raising=False)
...
def test_settings_upstream_websocket_proxy_env_auto_enables_when_dotenv_proxy_is_present(monkeypatch, tmp_path):
    ...
    for name in (
        "https_proxy",
        "HTTPS_PROXY",
        "AGENT_LB_UPSTREAM_WEBSOCKET_TRUST_ENV",
    ):
        monkeypatch.delenv(name, raising=False)
...
def test_settings_upstream_websocket_proxy_env_can_be_explicitly_disabled(monkeypatch):
    monkeypatch.setenv("https_proxy", "http://127.0.0.1:7890")
    monkeypatch.setenv("AGENT_LB_UPSTREAM_WEBSOCKET_TRUST_ENV", "false")
