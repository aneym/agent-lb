...
def test_chart_kube_version_floor_is_1_32() -> None:
    chart = yaml.safe_load(Path("deploy/helm/agent-lb/Chart.yaml").read_text(encoding="utf-8"))
    assert chart["kubeVersion"] == ">=1.32.0-0"


def test_chart_readme_documents_modern_support_policy() -> None:
    readme = Path("deploy/helm/agent-lb/README.md").read_text(encoding="utf-8")
...
def test_ci_uses_1_32_minimum_and_1_35_baseline() -> None:
    ...
    assert "kind create cluster --name agent-lb-smoke --image kindest/node:v1.35.0 --wait 120s" in makefile
